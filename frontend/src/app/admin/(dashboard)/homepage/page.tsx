    ...
    <div className="space-y-6 animate-in fade-in duration-500 w-full">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button 
          onClick={handleSave}
          disabled={isSaving}
          className="bg-[#5865f2] hover:bg-[#4752c4] text-white px-5 py-2.5 rounded-lg text-sm font-medium transition-colors flex items-center gap-2 shadow-sm border-0 disabled:opacity-70 cursor-pointer"
        >
      ...
      {successMessage && (
        <div className="bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 p-4 rounded-lg text-sm font-medium border border-emerald-200 dark:border-emerald-500/20 flex items-center shadow-sm">
      ...
      <div className="space-y-4">
        {sections.map(sec => (
          <div key={sec.id} className="bg-white dark:bg-[#14151a] rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden shadow-sm">
            <div 
                ...
                {sec.id === 'seo' ? (
                  <>
                    <div>
                      ...
                      <input type="text" value={config[sec.id]?.metaTitle || ''} onChange={e => updateSection(sec.id, 'metaTitle', e.target.value)} className="w-full px-3 py-2 bg-white dark:bg-[#14151a] border border-gray-200 dark:border-gray-800 rounded-lg text-sm focus:outline-none focus:border-[#5865f2]/50" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Meta Description</label>
                      <textarea rows={2} value={config[sec.id]?.metaDescription || ''} onChange={e => updateSection(sec.id, 'metaDescription', e.target.value)} className="w-full px-3 py-2 bg-white dark:bg-[#14151a] border border-gray-200 dark:border-gray-800 rounded-lg text-sm focus:outline-none focus:border-[#5865f2]/50" />
                    </div>
                  </>
                ) : (
                  <>
                    <div>
                      ...
                      <input type="text" value={config[sec.id]?.title || ''} onChange={e => updateSection(sec.id, 'title', e.target.value)} className="w-full px-3 py-2 bg-white dark:bg-[#14151a] border border-gray-200 dark:border-gray-800 rounded-lg text-sm focus:outline-none focus:border-[#5865f2]/50" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Nội dung / Mô tả</label>
                      <textarea rows={2} value={config[sec.id]?.description || config[sec.id]?.subtitle || ''} onChange={e => updateSection(sec.id, config[sec.id]?.subtitle !== undefined ? 'subtitle' : 'description', e.target.value)} className="w-full px-3 py-2 bg-white dark:bg-[#14151a] border border-gray-200 dark:border-gray-800 rounded-lg text-sm focus:outline-none focus:border-[#5865f2]/50" />
                    </div>
                  </>
                )}
              </div>
