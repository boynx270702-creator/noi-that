        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 transition-all duration-500">
          {filteredItems.map((item, index) => (
            <ScrollReveal key={item.id} animation="fade-up" delay={100 + index * 100}>
              <div
                className="group relative overflow-hidden bg-[#020b0d] shadow-sm dark:shadow-none border border-gray-100 dark:border-white/10 aspect-[4/3] animate-in fade-in zoom-in duration-500"
              >
                {/* Image Background */}
                <div
                ...
                <div className="absolute inset-0 p-8 flex flex-col justify-end translate-y-8 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500">
                  <h3 className="font-heading text-[22px] font-bold text-white mb-2">
                    <Link href={item.link} className="hover:text-[#27d6ff] transition-colors">
                      {item.title}
                    </Link>
                  </h3>
                  <p className="text-[#27d6ff] text-[13px] uppercase tracking-wider mb-6">
                    {item.categoryText}
                  </p>

                  {/* SVG Icon matching the original */}
                  <Link href={item.link} className="absolute bottom-8 right-8 w-10 h-10 bg-white dark:bg-[#131313] flex items-center justify-center hover:bg-[#27d6ff] transition-colors group/icon">
