      ...
      <div className="max-w-[1920px] mx-auto px-6">
        <div className="text-center mb-16">
          <h2 className="font-heading text-3xl md:text-5xl font-bold text-white mb-6 uppercase">
            Sản Phẩm <span className="text-[#27d6ff]">Bán Chạy</span>
          </h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {BEST_SELLERS.map((product) => (
            <div key={product.id} className="group flex flex-col bg-[#020b0d] shadow-sm dark:shadow-none border border-gray-100 dark:border-white/10 rounded-[4px] overflow-hidden hover:-translate-y-2 transition-all duration-300 luxury-glow shadow-sm shadow-black/20">
              <div className="relative aspect-[4/3] overflow-hidden">
                ...
                <div className="w-full h-full relative overflow-hidden">
                  ...
                  <div className="absolute inset-3 border border-[#27d6ff]/30 z-20 pointer-events-none transition-all duration-500 group-hover:opacity-0 group-hover:scale-105 rounded-[1px]"></div>
                </div>

                <div className="absolute top-4 left-4 flex gap-2 z-20">
                  {product.tags.map(tag => (
                    <span key={tag} className="bg-black/80 backdrop-blur-md text-[#27d6ff] text-[10px] font-bold uppercase tracking-widest px-3 py-1.5 rounded-[2px] luxury-glow">
                      {tag}
                    </span>
                  ))}
                </div>
              ...
              <div className="p-8 flex flex-col flex-grow relative">
                <div className="absolute top-0 right-8 -translate-y-1/2 w-12 h-12 modern-section border border-gray-200 dark:border-white/20 rounded-full flex items-center justify-center text-white group-hover:bg-[#27d6ff] group-hover:text-black group-hover:border-[#27d6ff] transition-colors shadow-sm">
                ...
                <h3 className="font-heading text-xl font-bold text-white mb-4">{product.name}</h3>

                <div className="mt-auto pt-4 border-t border-gray-200 dark:border-white/20 flex items-center justify-between">
                  <span className="text-[#27d6ff] text-sm font-bold">Đã bán {product.sold}+</span>
                  <Link href={`/san-pham/${product.id}`} className="text-sm text-white hover:text-[#27d6ff] font-semibold transition-colors flex items-center gap-1">
                    Xem chi tiết
        ...
        <div className="mt-16 text-center">
          <Link href="/san-pham?filter=ban-chay" className="inline-flex items-center justify-center border-2 border-[#27d6ff] text-[#27d6ff] hover:bg-[#27d6ff] hover:text-white px-8 py-4 rounded-[2px] font-bold uppercase tracking-widest transition-all text-sm text-white">
