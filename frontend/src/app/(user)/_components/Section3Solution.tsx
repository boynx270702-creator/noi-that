        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="relative order-2 lg:order-1">
            <ScrollReveal animation="fade-right" delay={100} className="relative z-10">
              <div className="relative aspect-[4/5] md:aspect-[3/4] lg:aspect-[4/5] w-full overflow-hidden">
                ...
                {/* Overlay Badge */}
                <div className="absolute bottom-10 left-10 right-10 bg-[#1bbce0] p-6 text-white transform translate-y-4 hover:translate-y-0 transition-transform duration-500">
            ...
            <div className="absolute -top-10 -left-10 w-full h-full border-2 border-[#1bbce0]/20 z-0" />
          </div>
          ...
          <div className="order-1 lg:order-2">
            <ScrollReveal animation="fade-left" delay={200}>
              ...
              <h2 className="font-heading text-3xl md:text-4xl lg:text-5xl font-bold text-[#1F1F1F] text-white mb-6 leading-tight">
                Chúng tôi giúp khách hàng chọn <span className="text-[#1bbce0]">đúng đơn vị</span> ngay từ đầu
              </h2>
              ...
              <div className="card bg-[#020b0d] p-8 border border-[#ECE7DE] dark:border-white/5">
                <h3 className="font-heading text-[#1F1F1F] text-white font-bold text-xl mb-6">Giá trị nhận được:</h3>
                <ul className="flex flex-col gap-4">
                  {VALUES.map((value, index) => (
                    <li key={index} className="flex items-start gap-4 group">
                      <div className="mt-1 bg-white/5 p-1 rounded-full group-hover:bg-[#1bbce0] transition-colors shrink-0">
                        <svg className="w-4 h-4 text-[#1bbce0] group-hover:text-[#1F1F1F] text-white transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7"></path></svg>
                      </div>
                      <span className="text-gray-600 dark:text-[#cccccc] text-[15px] leading-relaxed group-hover:text-[#1F1F1F] text-white transition-colors">
                        {value}
                      </span>
                    </li>
                  ))}
