    ...
    <section id="Process" className="relative py-32 bg-transparent dark:bg-transparent modern-section overflow-hidden">
      ...
      <div className="relative z-10 max-w-[1920px] mx-auto px-6">
        ...
        <div className="relative mt-12">
          {/* Central Line */}
          <div className="absolute left-[24px] md:left-1/2 top-0 bottom-0 w-[2px] bg-gradient-to-b from-[#1bbce0]/50 via-[#1bbce0]/20 to-transparent md:-translate-x-1/2" />
              ...
              >
                <div className={`flex flex-col md:flex-row items-start md:items-center w-full ${isEven ? 'md:flex-row-reverse' : ''}`}>
                  ...
                  <div className="w-full md:w-1/2 pl-16 md:pl-0 md:pr-16 md:text-right flex flex-col justify-center pt-2 md:pt-0">
                    <div className={`card dark:bg-[#131313] p-6 md:p-8 border border-[#ECE7DE] dark:border-white/5 hover:border-[#1bbce0]/50 transition-colors group relative rounded-[4px] shadow-sm ${isEven ? 'md:ml-16 md:text-left' : ''}`}>
                      <div className={`hidden md:block absolute top-4 ${isEven ? 'right-4' : 'left-4'} text-[#1bbce0] opacity-[0.03] text-7xl font-black group-hover:opacity-10 transition-opacity pointer-events-none`}>
                        {step.num}
                      </div>
                      <h3 className="font-heading text-[18px] md:text-[20px] font-bold text-[#1F1F1F] text-white mb-3 group-hover:text-[#1bbce0] transition-colors relative z-10">
                        {step.title}
                      </h3>
                  ...
                  <div className="absolute left-[24px] md:left-1/2 top-4 md:top-1/2 w-10 h-10 rounded-full bg-[#FFFFFF] dark:bg-[#0a0a0a] border-2 border-[#1bbce0] flex items-center justify-center text-[#1bbce0] font-bold text-[15px] md:-translate-x-1/2 md:-translate-y-1/2 z-10 shadow-[0_0_20px_rgba(206,158,81,0.3)] -translate-x-1/2">
                    {index + 1}
                  </div>
