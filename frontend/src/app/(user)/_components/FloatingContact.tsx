...
export default function FloatingContact() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[9999] flex flex-col gap-4">
      ...
      <button
        onClick={goToTop}
        className={`w-[52px] h-[52px] bg-[#1F1F1F] dark:bg-white text-white dark:text-[#1F1F1F] rounded-full flex items-center justify-center shadow-lg hover:bg-[#1bbce0] dark:hover:bg-[#1bbce0] dark:hover:text-white transition-all duration-300 relative group overflow-hidden ${showTopBtn ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10 pointer-events-none'}`}
      ...
      >
        ...
        <span className="absolute right-full mr-4 bg-[#020b0d] text-white text-[13px] px-3 py-1.5 rounded-[4px] shadow-md opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all whitespace-nowrap border border-gray-100 dark:border-white/10 font-medium">
          Lên đầu trang
        </span>
      </button>
