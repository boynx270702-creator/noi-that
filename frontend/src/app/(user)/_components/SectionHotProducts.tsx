      ...
      <div className="max-w-[1400px] mx-auto px-6 relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
          <div className="max-w-2xl">
            ...
            <h2 className="font-heading text-4xl md:text-5xl lg:text-6xl font-bold text-[#1F1F1F] dark:text-white leading-[1.1]">
              Tuyệt Tác Nội Thất <br/>
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#C7A25C] to-[#E2C792] italic font-medium">Xứng Tầm Không Gian</span>
            </h2>
          </div>
          ...
          <div className="flex -ml-6">
            {HOT_PRODUCTS.map((product) => (
              <div key={product.id} className="flex-[0_0_85%] sm:flex-[0_0_45%] lg:flex-[0_0_28%] xl:flex-[0_0_24%] pl-6">
                <Link href={`/san-pham/${product.id}`} className="group block relative w-full overflow-hidden rounded-[8px] bg-gray-100 dark:bg-[#131313]">
                  {/* Aspect Ratio 3:4 */}
                  <div className="relative aspect-[3/4] w-full overflow-hidden">
                    <img 
                      src={product.image} 
                      alt={product.name}
                      className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-105 luxury-image-filter"
                    />
