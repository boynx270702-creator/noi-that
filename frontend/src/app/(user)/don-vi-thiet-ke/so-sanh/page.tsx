...
export default function SoSanhPage({ searchParams }: { searchParams: Promise<{ ids?: string }> }) {
  ...
  if (isLoading) {
    return <div className="overflow-hidden relative pt-[120px] pb-20 modern-section min-h-screen text-[#1F1F1F] text-white flex items-center justify-center">
      <SectionStarryMotif position="random-corner" />
      <div className="flex flex-col items-center gap-4">
        <svg className="animate-spin h-10 w-10 text-[#1bbce0]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
  ...
  return (
    <div className="pt-[120px] pb-20 modern-section min-h-screen text-[#1F1F1F] text-white">
      <div className="container mx-auto px-4 lg:px-8 max-w-[1440px]">
        {/* Header */}
        <div className="flex flex-wrap items-center justify-between mb-10 gap-4">
          <div>
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-[2px] bg-gradient-to-r from-[#1bbce0]/20 to-transparent border-l-2 border-[#1bbce0] text-[#A67C00] dark:text-[#FFD700] text-[11px] font-bold uppercase tracking-widest mb-4 luxury-glow">
              So sánh năng lực
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-12 relative">
          ...
          {/* V.S Badge Center */}
          {unitA && unitB && (
            <div className="hidden lg:flex absolute top-[15%] left-1/2 -translate-x-1/2 -translate-y-1/2 w-16 h-16 bg-[#1F1F1F] dark:bg-white text-[#1bbce0] dark:text-[#1F1F1F] rounded-full items-center justify-center font-heading text-2xl font-bold z-10 border-4 border-[#ECE7DE] dark:border-[#131313] shadow-[0_0_30px_rgba(199,162,92,0.3)]">
              VS
            </div>
          )}
          ...
          {unitA ? (
            <div className="card dark:bg-[#1c1c1c] rounded-[4px] border border-[#ECE7DE] dark:border-white/10 shadow-sm p-6 relative overflow-hidden">
              ...
              <div className="h-64 w-full mb-6 relative rounded-[2px] overflow-hidden group">
                ...
                <div className="absolute bottom-4 left-4 right-4">
                  <div className={`inline-block text-[10px] font-bold px-3 py-1 rounded-[2px] uppercase tracking-widest mb-2 shadow-sm ${
                    unitA.category.includes('Cao cấp') ? 'bg-gradient-to-r from-[#27d6ff] to-[#E5C98A] text-[#131313]' :
              ...
              <div className="space-y-8">
                ...
                <div>
                  <div className="flex justify-between text-sm mb-2 font-bold uppercase tracking-wider text-gray-500 text-white/60">
                    <span>Kinh nghiệm</span>
                    <span className="text-[#1F1F1F] text-white">{unitA.experience}</span>
                  </div>
                  <div className="h-2 w-full bg-gray-200 dark:bg-white/10 rounded-full overflow-hidden">
                    <div className="h-full bg-gradient-to-r from-[#1bbce0] to-[#E5C98A] rounded-full" style={{ width: `${Math.min((unitA.experienceNum / 20) * 100, 100)}%` }}></div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="col-span-2 p-4 bg-gray-50 dark:bg-white/5 rounded-[2px] border border-[#ECE7DE] dark:border-white/5">
                    ...
                    <div className="flex items-center gap-2 font-bold">
                      <svg className="w-4 h-4 text-[#1bbce0]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>
                      {unitA.location}
                    </div>
                  </div>
                </div>
                  ...
                  <ul className="space-y-3">
                    {unitA.services.map((s: string, i: number) => (
                      <li key={i} className="flex items-center gap-3">
                        <div className="w-5 h-5 rounded-full bg-[#1bbce0]/20 text-[#1bbce0] flex items-center justify-center shrink-0">
                ...
                <div className="pt-6 mt-6 border-t border-[#ECE7DE] dark:border-white/10 flex gap-4">
                  <Link href={`/don-vi-thiet-ke/${unitA.id}`} className="flex-1 text-center border border-[#1F1F1F] dark:border-white text-[#1F1F1F] text-white font-bold py-4 px-4 rounded-[2px] transition-colors uppercase tracking-wider text-sm hover:bg-gray-50 dark:hover:bg-white/5">
                    Hồ sơ
                  </Link>
                  <Link href={`/tu-van?unit=${unitA.id}`} className="flex-[2] text-center bg-[#1bbce0] text-white font-bold py-4 px-4 rounded-[2px] transition-colors uppercase tracking-wider text-sm hover:bg-[#b08e4f] shadow-[0_0_20px_rgba(199,162,92,0.3)]">
                    Nhận tư vấn
                  </Link>
                </div>
              </div>
          ...
          {unitB ? (
            <div className="card dark:bg-[#1c1c1c] rounded-[4px] border border-[#ECE7DE] dark:border-white/10 shadow-sm p-6 relative overflow-hidden">
              ...
              <div className="h-64 w-full mb-6 relative rounded-[2px] overflow-hidden group">
                ...
                <div className="absolute bottom-4 left-4 right-4">
                  <div className={`inline-block text-[10px] font-bold px-3 py-1 rounded-[2px] uppercase tracking-widest mb-2 shadow-sm ${
                    unitB.category.includes('Cao cấp') ? 'bg-gradient-to-r from-[#27d6ff] to-[#E5C98A] text-[#131313]' :
              ...
              <div className="space-y-8">
                ...
                <div>
                  <div className="flex justify-between text-sm mb-2 font-bold uppercase tracking-wider text-gray-500 text-white/60">
                    <span>Kinh nghiệm</span>
                    <span className="text-[#1F1F1F] text-white">{unitB.experience}</span>
                  </div>
                  <div className="h-2 w-full bg-gray-200 dark:bg-white/10 rounded-full overflow-hidden">
                    <div className="h-full bg-gradient-to-r from-[#1bbce0] to-[#E5C98A] rounded-full" style={{ width: `${Math.min((unitB.experienceNum / 20) * 100, 100)}%` }}></div>
                  </div>
                </div>
                ...
                <div className="pt-6 mt-6 border-t border-[#ECE7DE] dark:border-white/10 flex gap-4">
                  <Link href={`/don-vi-thiet-ke/${unitB.id}`} className="flex-1 text-center border border-[#1F1F1F] dark:border-white text-[#1F1F1F] text-white font-bold py-4 px-4 rounded-[2px] transition-colors uppercase tracking-wider text-sm hover:bg-gray-50 dark:hover:bg-white/5">
                    Hồ sơ
                  </Link>
                  <Link href={`/tu-van?unit=${unitB.id}`} className="flex-[2] text-center bg-[#1bbce0] text-white font-bold py-4 px-4 rounded-[2px] transition-colors uppercase tracking-wider text-sm hover:bg-[#b08e4f] shadow-[0_0_20px_rgba(199,162,92,0.3)]">
                    Nhận tư vấn
                  </Link>
                </div>
              </div>
            ...
            <div 
              ...
              className="card dark:bg-[#1c1c1c] rounded-[4px] border-2 border-dashed border-[#ECE7DE] dark:border-white/20 p-12 flex flex-col items-center justify-center text-center cursor-pointer hover:border-[#1bbce0] hover:bg-[#1bbce0]/5 transition-all group min-h-[600px]"
            >
              <div className="w-24 h-24 rounded-full bg-[#ECE7DE] dark:bg-white/10 flex items-center justify-center text-[#1F1F1F]/40 text-white/40 group-hover:bg-[#1bbce0] group-hover:text-white group-hover:shadow-[0_0_30px_rgba(199,162,92,0.4)] transition-all mb-6">
              ...
              <p className="text-gray-500 text-white/60">Chọn thêm một đơn vị thiết kế khác để đặt lên bàn cân so sánh chi tiết năng lực và dịch vụ.</p>
            </div>
          )}
            ...
            <div className="p-6 border-b border-[#ECE7DE] dark:border-white/10">
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full bg-gray-100 dark:bg-white/5 border border-transparent dark:border-white/10 py-4 pl-12 pr-4 rounded-[2px] outline-none focus:border-[#1bbce0] transition-colors"
                />
              </div>
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {availableUnits.map(unit => (
                    <div 
                      ...
                      className="flex items-center gap-4 p-4 border border-[#ECE7DE] dark:border-white/10 rounded-[2px] hover:border-[#1bbce0] cursor-pointer group transition-colors"
                    >
                      <div className="w-16 h-16 rounded-[2px] bg-cover bg-center shrink-0" style={{ backgroundImage: `url(${unit.avatar})` }}></div>
                      <div>
                        <div className="text-[10px] text-[#1bbce0] font-bold uppercase tracking-wider mb-1">{unit.category}</div>
                        <h4 className="font-bold group-hover:text-[#1bbce0] transition-colors">{unit.name}</h4>
